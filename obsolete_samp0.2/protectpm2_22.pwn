new mode_name[] = "Protect the PM 2 v2.2";
...
//          writeToIrc(dest, UserList);
			...
			if (streq(ircCmd,"!adminsay")) {
				...
				GameTextForAll2(ircRest,4000,4);
				return 1;
...
AddBannedCuboid(Float:max_x, Float:min_x, Float:max_y, Float:min_y, Float:max_z, Float:min_z, message[], Float:tele_x, Float:tele_y, Float:tele_z, Float:tele_a, vehicles, interior)
{
...
public MapVoteDisplayNextMap()
{
	...
	switch (map) {
		...
		case 0 : format(text, sizeof text, "Next map: Los Santos with Hydra!");
		case 1 : format(text, sizeof text, "Next map: San Fierro!");
		case 2 : format(text, sizeof text, "Next map: Desert!");
		case 3 : format(text, sizeof text, "Next map: Area 51!");
		case 4 : format(text, sizeof text, "Next map: Las Venturas with Objectives!");
		case 5 : format(text, sizeof text, "Next map: Air Assault!");
		case 6 : format(text, sizeof text, "Next map: Mount Chiliad!");
		case 7 : format(text, sizeof text, "Next map: Countryside!");
		case 8 : format(text, sizeof text, "Next map: Los Santos!");
		case 9 : format(text, sizeof text, "Next map: Las Venturas!");
		case 10 : format(text, sizeof text, "Next map: Factory!");
		case 11 : format(text, sizeof text, "Next map: Bayside!");
	}
	SendClientMessageToAll2(COLOUR_GLOBAL,text);
}
...
//		AddStaticVehicle2(V_SEASPAR,-442.7218,-2665.1450,155.4987,137.0493,-1,-1);
		...
		AddStaticVehicle2(402,-2245.3557,-1737.0740,479.7292,14.1842,30,30); // buffalo
		AddStaticVehicle2(444,-1795.8589,-2344.1697,35.8757,156.9109,32,53); // monster
		AddStaticVehicle2(444,-1865.8369,-2650.0090,55.6515,258.8762,32,53); // monster
		AddStaticVehicle2(560,-215.2989,-2815.4988,45.2884,325.1967,33,0); // sultan

		AddStaticVehicle2(560,-382.8942,-2820.9539,45.0236,81.8549,33,0); // sultan
		AddStaticVehicle2(560,-634.6548,-2795.6040,49.6211,93.4775,33,0); // sultan
		AddStaticVehicle2(560,-1388.8885,-2917.0947,47.1867,15.3225,33,0); // sultan
		AddStaticVehicle2(400,-2844.6697,-892.7352,9.2927,114.0765,101,1); // landstalker

		AddStaticVehicle2(402,-128.4595,-972.0978,26.1023,81.9887,22,22); // buffalo
		AddStaticVehicle2(402,-334.1578,-848.3418,47.0545,62.6623,22,22); // buffalo
		AddStaticVehicle2(470,-1139.3645,-1414.8688,126.1385,291.0272,43,0); // patriot
		AddStaticVehicle2(470,-1034.6451,-1174.2344,129.2108,90.0000,43,0); // patriot
		AddStaticVehicle2(470,-1033.2537,-1167.2495,129.2133,90.0190,43,0); // patriot
		AddStaticVehicle2(470,-879.5825,-1243.6193,111.6640,307.1497,43,0); // patriot

		AddStaticVehicle2(470,-549.8234,-936.0164,58.7215,317.1906,43,0); // patriot
		AddStaticVehicle2(470,-568.9822,-1048.6821,23.8613,237.0704,43,0); // patriot
		AddStaticVehicle2(470,-567.1197,-1045.0377,23.9362,237.0529,43,0); // patriot
		AddStaticVehicle2(474,-74.5151,-1589.3416,2.3799,248.3364,110,1); // hermes
		AddStaticVehicle2(474,-254.6894,-1698.9756,2.6274,171.3307,110,1); // hermes
		AddStaticVehicle2(474,-280.0861,-1802.5422,13.7244,300.3557,110,1); // hermes

		AddStaticVehicle2(474,-387.6492,-1984.9158,27.1313,339.0298,110,1); // hermes
		AddStaticVehicle2(474,-196.9525,-2402.9727,34.5856,219.0458,110,1); // hermes
		AddStaticVehicle2(439,-179.2676,-2692.4382,35.1404,170.1560,67,8); // stallion
		AddStaticVehicle2(439,-503.0177,-2618.2209,162.4671,326.4368,67,8); // stallion
		AddStaticVehicle2(500,-578.3273,-2357.9270,27.3549,323.7466,21,119); // mesa
		AddStaticVehicle2(500,-657.2610,-2201.4683,11.1079,317.4389,21,119); // mesa
		AddStaticVehicle2(500,22.1420,-1296.6606,13.1033,130.3329,21,119); // mesa

		AddStaticVehicle2(471,-2234.0500,-2056.3655,119.5490,121.3874,120,112); // quadbike
		AddStaticVehicle2(471,-2351.0264,-2105.7593,112.1116,97.6164,120,112); // quadbike
		AddStaticVehicle2(424,-2445.3860,-2564.5083,51.4009,21.0933,6,16); // bfinjection
		AddStaticVehicle2(424,-2247.6382,-2671.7041,58.5060,240.7580,6,16); // bfinjection
		AddStaticVehicle2(424,-2012.4364,-2637.6135,58.0434,279.4527,6,16); // bfinjection
...
Motd(playerid)
{
	...
	SendClientMessage(playerid,COLOUR_IMPORTANT,"(All heavy weapon pickups are disabled when there are 8 or less players)");
	SendClientMessage(playerid,COLOUR_IMPORTANT,"(Security cameras and player map markers changed in Factory)");
	SendClientMessage(playerid,COLOUR_IMPORTANT,"(More objects added to the factory in Countryside)");
 	SendClientMessage(playerid,COLOUR_IMPORTANT,"(Forum: http://www.sparkptpm.co.uk/  IRC: irc.gtanet.com(6667) #Ptpm #ptpm.echo)");
...
/*			if (distance_display && pm!=INVALID_PLAYER_ID) {
				...
				GetPlayerName(pm,name,sizeof name);
				if (IsPlayerConnected(i) && player_class[i]!=INVALID_CLASS) {
					if (class_team[player_class[i]]!=TEAM_PSYCHO) {
	...
	if (pickup_weaponid[pickupid]==WEAPON_MINIGUN ||
		...
		pickup_weaponid[pickupid]==WEAPON_HEATSEEKER) {
  		if (NumPlayersConnected()<=8) {
			GameTextForPlayer(playerid,"~w~Heavy weapons disabled while there are 8 or less players",6000,GAME_TEXT_STYLE_SMALL);
			return;
		}
	}
