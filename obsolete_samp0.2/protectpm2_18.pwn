new mode_name[] = "Protect the PM 2 v1.8";
...
// AddSecurityCamera();
		...
		class_psycho1 = AddPlayerClass2(230, WEAPON_PARACHUTE,-1, WEAPON_CHAINSAW,-1, WEAPON_AK47,80);
		class_psycho2 = AddPlayerClass2(212, WEAPON_PARACHUTE,-1, WEAPON_CHAINSAW,-1, WEAPON_AK47,80);
		class_psycho3 = AddPlayerClass2(200, WEAPON_PARACHUTE,-1, WEAPON_KNIFE,-1, WEAPON_AK47,80);
		class_psycho4 = AddPlayerClass2(137, WEAPON_PARACHUTE,-1, WEAPON_KNIFE,-1, WEAPON_AK47,80);
...
Motd(playerid)
{
	...
	SendClientMessage(playerid,COLOUR_IMPORTANT,"(Spawn ammo reduced for psychopaths)");
	SendClientMessage(playerid,COLOUR_IMPORTANT,"(Ammo reduced on Sniper rifle and Grenade pickups)");
	SendClientMessage(playerid,COLOUR_IMPORTANT,"(Players will now recieve ammo boosts for consecutive kills)");
...
public OnPlayerConnect(playerid)
{
	...
	if (round_timer_counter > 0) {
		...
		SendClientMessageToAll(COLOUR_CONNECTION,text);
	}
	Motd(playerid);
...
public OnPlayerDeath(playerid, killerid, reason)
{
	...
	if (killerid!=INVALID_PLAYER_ID) {
	    if (player_consecutive_kills[playerid] >= 3) {
	        ...
	        SendClientMessageToAll(COLOUR_ACHIEVEMENT,text);
		}
	}
...
public RoundTick()
{
	...
	for(new i=0;i<playerid_max;i++) {
		GetPlayerPos(i,x_pos,y_pos,z_pos);
		if (x_pos>=-2664.2251 &&
 			x_pos<=-2658.3267 &&
			y_pos>=1424.9188 &&
			y_pos<=1434.9404 &&
...
//		SetPlayerClass(playerid,victim_class);
	...
	if (IsPlayerOp(playerid)) {
		if (streq(cmd,"/kick")) {
			...
			WriteBanToFile("ip_ban",banner,reason,ip);
			return 1;
			...
			SendClientMessageToAll(COLOUR_PERSONAL,result);
			SendRconCommand("reloadbans 0");
			printf(result);
	...
	if (pickup_unauthorised[class_team[player_class[playerid]]][pickupid]) {
		...
		if (pickup_weaponid[pickupid]==WEAPON_MINIGUN || pickup_weaponid[pickupid]==WEAPON_ROCKETLAUNCHER || pickup_weaponid[pickupid]==WEAPON_HEATSEEKER) {
			...
			if (pickup_weaponid[pickupid]==WEAPON_MINIGUN) {
				...
				format(text,sizeof text,"~w~%s has the ~y~rocket launcher!",text);
			} else if (pickup_weaponid[pickupid]==WEAPON_HEATSEEKER) {
                player_legal_heatseeker[playerid] = 1;
				format(text,sizeof text,"~w~%s has the ~y~heat seeker!",text);
			}
...
AntiSpam(playerid, text[])
{
    ...
    if (player_time[playerid][count] > (time - SPAM_TIME)) {
		...
		if (player_repeat_count[playerid] == 1) {
			...
			SendClientMessageToAll(COLOUR_GLOBAL,message);
			Kick(playerid);
		}
		return;
	} else {
...
IpMatch(ip1[], ip2[])
{
	...
	format(ip1, 15,"%s.%s.%s.%s",ip1text[0],ip1text[1],ip1text[2],ip1text[3]);
	format(ip2, 15,"%s.%s.%s.%s",ip2text[0],ip2text[1],ip2text[2],ip2text[3]);
