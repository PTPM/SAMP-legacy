new mode_name[] = "Protect the PM 2 v2.0";
...
//          new user_list[20][254];
            ...
            for (new i=0 ; i<playerid_max ; i++) {
                if (IsPlayerConnected(i)) {
                    new name[25];
                    GetPlayerName(i,name,sizeof name);
					format(user_list[line_count], 254, "%s %d%s [%d] ,", user_list[line_count],getIrcClassColor(i), name,i);
                    p_count++;
                    if (p_count%10==0) {
                            if (line_count==0)
...
//          writeToIrc(dest, UserList);
			...
			if (streq(ircCmd,"!adminsay")) {
				...
				format(ircMsg, sizeof ircMsg, "NOTICE @%s 11 %s", EchoChannel, text2);
...
Motd(playerid)
{
	...
	SendClientMessage(playerid,COLOUR_IMPORTANT,"(Security cameras, player map markers and PM intel changed in Factory)");
	SendClientMessage(playerid,COLOUR_IMPORTANT,"(Boundaries extended to the south west around the factory in Countryside)");
	SendClientMessage(playerid,COLOUR_IMPORTANT,"(Moving platforms added to Las Venturas)");
...
public OnPlayerDeath(playerid, killerid, reason)
{
	...
	if(playerid == killerid){
		format(text, sizeof text, "4** %d%s 4killed himself (%s).", getIrcClassColor(playerid), victimName, ReturnWeaponName(reason));
	}else{
	    GetPlayerName(killerid, killerName, sizeof killerName);
		format(text, sizeof text, "4** %d%s 4got killed by %d%s 4(%s).", getIrcClassColor(playerid), victimName, getIrcClassColor(killerid), killerName, ReturnWeaponName(reason));
	}
	...
	if (class_team[player_class[playerid]]==TEAM_PRIMEMINISTER) {
		...
		format(deathCause, sizeof deathCause,"8** %s", deathCause);
...
public OnPlayerText(playerid,text[])
{
	...
	format(text2,sizeof text2, "[%d]%d %s: %s",playerid,getIrcClassColor(playerid), text2, text);
...
PrintVoteInfo()
{
	...
	if (votes_left){
        ...
        format(ircInfo,sizeof ircInfo, "11* Callvote: 4ban %s (id %d)?, 3Reason: %s, 10need %d votes",name,vote_victim,vote_reason,votes_left);
	}else{
		format(info,sizeof info,"%s (id %d) voted off the server!",name,vote_victim);
		format(ircInfo,sizeof ircInfo, "11* Callvote: %s (id %d) voted off the server!",name,vote_victim);
	}
...
SendClientMessageToAdmins(colour,message[])
{
	...
	if(streq(GetHEXString(colour),"8080FFAA")){
		format(ircMsg, sizeof ircMsg, "11 %s",message);
	}else if(streq(GetHEXString(colour),"FF0000AA")){
		format(ircMsg, sizeof ircMsg, "4 %s",message);
		...
		format(ircMsg, sizeof ircMsg, "5 %s",message);
	}else if(streq(GetHEXString(colour),"C0C0C0FF")){
		format(ircMsg, sizeof ircMsg, "15 %s",message);
	}else if(streq(GetHEXString(colour),"5EAA02FF")){
		format(ircMsg, sizeof ircMsg, "9 %s",message);
	}
...
GameTextForAll2(text[], time, style){
    ...
    new ircMsg[256];
    format(ircMsg, sizeof ircMsg, "14*** %s", text);
    ircMsg = strreplace("~n~", "",ircMsg);
    ircMsg = strreplace("~w~", "",ircMsg);
	ircMsg = strreplace("~y~", "8 ",ircMsg);
	ircMsg = strreplace("~g~", "3 ",ircMsg);
	ircMsg = strreplace("~b~", "10 ",ircMsg);
	ircMsg = strreplace("~p~", "6 ",ircMsg);
    ircMsg = strreplace("~r~", "4 ",ircMsg);
    writeToIrc(EchoChannel, ircMsg);
    return 1;
}
