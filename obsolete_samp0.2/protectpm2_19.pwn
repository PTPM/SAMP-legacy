new mode_name[] = "Protect the PM 2 v1.9";
		...
		if (streq(ircCmd,"!cmds")) {
			...
			for (new i=0 ; i<playerid_max ; i++) {
				if (IsPlayerConnected(i)) {
					new name[25];
					GetPlayerName(i,name,sizeof name);
					format(UserList[line_count], 254, "%s %d%s [%d] ,", UserList[line_count],getIrcClassColor(i), name,i);
					p_count++;
					if (p_count%10==0) {
					    if (line_count==0)
					    	format(UserList[line_count], 254, "\2;Users\2; (%d): %s",NumPlayersConnected(),UserList[line_count]);
						writeToIrc(dest, UserList[line_count]);
						line_count++;
					}
				}
			}
...
Motd(playerid)
{
	...
	SendClientMessage(playerid,COLOUR_IMPORTANT,"(Bans are NOT lifted every day anymore (see forum for more info))");
	SendClientMessage(playerid,COLOUR_IMPORTANT,"(IRC echo added - join #ptpm.echo to see more)");
	SendClientMessage(playerid,COLOUR_IMPORTANT,"(Back entrance the to factory in Los Santos has been blocked)");
	SendClientMessage(playerid,COLOUR_IMPORTANT,"(Sea Sparrow removed from and Sinking Ship added to Chiliad.)");
 	SendClientMessage(playerid,COLOUR_IMPORTANT,"(PTPM forum: http://www.sparkptpm.co.uk/  IRC: irc.gtanet.com(6667) #Ptpm)");
