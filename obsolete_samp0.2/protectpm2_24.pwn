new mode_name[] = "Protect the PM 2 v2.4";
...
//	    ircConnect("extreme.nl.eu.gtanet.com", 6667, "Grimlick", 1);
...
//          writeToIrc(dest, UserList);
			...
			format(text,sizeof text, "name: %s [%d] (%s) cl:%d %s (%.2f,%.2f,%.2f) tk:%d %s ping:%d", name, id, player_username[id], player_class[id], team, x,y,z,player_teamkills[id], ((player_legal_minigun[id])?("mini"):("no_mini")), GetPlayerPing(id));
...
Motd(playerid)
{
	...
	SendClientMessage(playerid,COLOUR_IMPORTANT,"- /tasks commands added");
	SendClientMessage(playerid,COLOUR_IMPORTANT,"- miscellaneous fixes & tweaks");
	SendClientMessage(playerid,COLOUR_IMPORTANT,"- objects added to LS");
	SendClientMessage(playerid,COLOUR_IMPORTANT,"- textdraw added to Factory");
	SendClientMessage(playerid,COLOUR_IMPORTANT,"- objects & cuboids added to north ship in SF");
	...
	if (IsPlayerOp(playerid)) {
		if (streq(cmd,"/kick")) {
			...
			return 1;
  			...
  			ircConnect("frosties.de.eu.gtanet.com", 6667, "Grimlock", 1);
			ircConnect("frosties.de.eu.gtanet.com", 6667, "Grimlack", 2);
			return 1;
