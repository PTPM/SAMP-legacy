new mode_name[] = "Protect the PM 2 v1.3";
...
new pm_current_interior = 0;
new pm_previous_interior = 0;
...
*/
		...
		AddWeaponPickup(PICKUP_ROCKETLAUNCHER,WEAPON_ROCKETLAUNCHER,20,-427.36,2505.86,124.30,90,WP_SYNCED);
		AddWeaponPickup(PICKUP_HEATSEEKER,WEAPON_HEATSEEKER,20,248.47,1457.66,43.09,90,WP_SYNCED);
...
Motd(playerid)
{
	...
	SendClientMessage(playerid,COLOUR_IMPORTANT,"(Set armour to 0 on spawn)");
...
public RoundTick()
{
	...
	pm_current_interior = GetPlayerInterior(pm);
	GetPlayerPos(pm,pm_x,pm_y,pm_z);
	if (pm_current_interior != pm_previous_interior) {
	    if (pm_current_interior == 0) {
	        for(new i=0; i<playerid_max; i++) {
	        	RemovePlayerMapIcon(i,1);
			}
		} else if (pm_current_interior != 0) {
		    for(new i=0; i<playerid_max; i++) {
	    		SetPlayerMapIcon(i,1,pm_x,pm_y,pm_z,60,0xffffffAA);
			}
		}
	}
 	pm_previous_interior = pm_current_interior;
	...
	if (pickup_unauthorised[class_team[player_class[playerid]]][pickupid]) {
		...
		if (pickup_weaponid[pickupid]==WEAPON_MINIGUN || pickup_weaponid[pickupid]==WEAPON_ROCKETLAUNCHER || pickup_weaponid[pickupid]==WEAPON_HEATSEEKER) {
			...
			if (pickup_weaponid[pickupid]==WEAPON_MINIGUN) {
				...
				format(text,sizeof text,"~w~%s has the ~y~rocket!",text);
