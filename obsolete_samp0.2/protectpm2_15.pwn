new mode_name[] = "Protect the PM 2 v1.5";
...
new city_weather[NUM_CITY_WEATHER] = {
	1,
	2,
	7,
	8,
...
new country_weather[NUM_COUNTRY_WEATHER] = {
	4,
	8,
	1,
	9,
...
new desert_weather[NUM_DESERT_WEATHER] = {
	3,
	12,
	14,
	17,
...
#define SPAM_LIMIT 6 // players are allowed this many repeats before they are muted
#define SPAM_MUTE_LENGTH 30 // length of the mute
...
//		AddStaticPickup(PICKUP_PARACHUTE,2,1529.0437,-1364.1820,327.8988); // parachute
		...
		EnableTirePopping(1);
		SetWeather(city_weather[city_weather_start]);
...
*/
        ...
        AllowInteriorWeapons(1);
		EnableTirePopping(1);
		SetWeather(desert_weather[desert_weather_start]);
...
//  		AddGameBoundaryCorner(-959,-3000);
		...
		AllowInteriorWeapons(1);
		EnableTirePopping(1);
		SetWeather(country_weather[country_weather_start]);
...
Motd(playerid)
{
	...
	SendClientMessage(playerid,COLOUR_IMPORTANT,"(NEW: Weather system tweaked)");
...
AntiSpam(playerid, text[])
{
	...
	if (streq(previous_text[playerid],text)) {
	    ...
	    if (player_repeat[playerid] == SPAM_LIMIT+1) {
	    	player_mute_time[playerid] = SPAM_MUTE_LENGTH;
	    	player_muted[playerid] = 1;
	    	new message[256];
			format (message,sizeof message,"~g~You have been muted for %d seconds for repeating.",SPAM_MUTE_LENGTH);
