...
new mode_name[] = "Protect the Prime Minister 2.7";
...
public OnPlayerConnect(playerid)
{
	...
	SendClientMessage(playerid,COLOUR_IMPORTANT,"(Teamkillers are now punished)");
...
public RoundTick()
{
	...
	for (new i=0 ; i<MAX_PLAYERS ; i++) {
	    if (IsPlayerConnected(i) && player_teamkill_freeze_counter[i]>0) {
	        player_teamkill_freeze_counter[i]--;
...
CancelVote(playerid)
{
	if (vote_victim!=INVALID_PLAYER_ID && playerid==vote_initiator) {
		...
		vote_victim = INVALID_PLAYER_ID;
	} else if (vote_victim!=INVALID_PLAYER_ID && IsPlayerAdmin(playerid)) {
