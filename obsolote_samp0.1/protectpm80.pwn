new mode_name[] = "Protect the Prime Minister 8.0";
...
{
	SendClientMessage(playerid,COLOUR_PERSONAL,"Usage: /mapvote ls_hydra|sf|desert|a51|lv_obj|air|chiliad|country|ls|lv");
}
...
public PlayerMapVoteStatus(playerid)
{
	...
	new text[256];
	format(text, sizeof text,"Current scores: ls_hydra:%d, sf:%d, desert:%d, a51:%d, lv_obj:%d, air:%d chiliad:%d country:%d ls:%d lv:%d", score[0], score[1], score[2], score[3], score[4],score[5],score[6],score[7],score[8],score[9]);
	...
	*/
	...
	if (!transmap) {
		...
		for (new i=0 ; i<MAX_PLAYERS ; i++) {
			...
			for (new j=0 ; j<MAX_USERNAME ; j++)
		    	if (line[j]=='\n' || line[j]=='\r') line[j]='\0';
		    player_username[i] = line;
		    if (!streq(line,"")) {
...
public OnPlayerConnect(playerid)
{
	...
	SendClientMessage(playerid,COLOUR_IMPORTANT,"(NEW: original LS & LV maps moved to ls_hydra and lv_obj)");
	SendClientMessage(playerid,COLOUR_IMPORTANT,"(NEW: ls & lv maps now have rules like SF)");
...
public OnPlayerCommandText(playerid, cmdtext[])
{
	...
	if (numparams==0) {}
 	...
 	if (streq(cmd,"/reclass")) {
		...
		format(text,sizeof text, "name: %s [%d] (%s) cl:%d %s (%.2f,%.2f,%.2f) d:%.2f tk:%d %s", text, id, player_username[id], player_class[id], team, x,y,z, DistanceFromPlayerToPlayer(playerid,id),player_teamkill_freeze_counter[id],player_legal_minigun[id]?"mini":"no_mini");
...
PlayerLogin(playerid,username[],password[])
{
	...
	new text[256];
	GetPlayerName(playerid,text,sizeof text);
	printf("Player %s (%d) logged in as %s\n",text,playerid,username);
	log_printf("Login: %d %s as %s",playerid,text,username);
	format (text,sizeof text,"%s (%d) logged in as %s",text,playerid,username);
	if (IsPlayerOp(playerid))
	    format(text,sizeof text, "%s (an operator)",text);
	SendClientMessageToAll(COLOUR_GLOBAL,text);
