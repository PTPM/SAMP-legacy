new mode_name[] = "Protect the Prime Minister 3.6";
...
new pickup_respawn_time[MAX_PICKUPS];
...
public OnPlayerConnect(playerid)
{
	...
	SendClientMessage(playerid,COLOUR_IMPORTANT,"(More pickups for bodyguards on a51)");
	SendClientMessage(playerid,COLOUR_IMPORTANT,"(Tweaked pickup code a little)");
	SendClientMessage(playerid,COLOUR_IMPORTANT,"(Tweaked speed cheat code a little)");
	...
	switch (newstate) {
		case PLAYER_STATE_ONFOOT: {
			...
			for (new i=0 ; i<NUM_PLOTS ; i++) {
				...
				player_plot_y[playerid][i] = 0;  // intention is to disrupt the track when exiting vehicle
		...
		case PLAYER_STATE_WASTED: {
			...
			for (new i=0 ; i<NUM_PLOTS ; i++) {
				player_plot_x[playerid][i] = 0;
				player_plot_y[playerid][i] = 0;  // intention is to disrupt the track when exiting vehicle
				player_plot_y[playerid][i] = 0;  // (you also exit vehicles when you die!)
			}
		}
