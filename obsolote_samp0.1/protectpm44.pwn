new mode_name[] = "Protect the Prime Minister 4.4";
...
public OnPlayerConnect(playerid)
{
	...
	SendClientMessage(playerid,COLOUR_IMPORTANT,"(Various minor improvements from r3mp and sniper)");
...
public OnPlayerCommandText(playerid, cmdtext[])
{
	...
	if (player_op[playerid]) {
		if (streq(cmd,"/kick")) {
			...
			player_watching[playerid] = INVALID_PLAYER_ID;
			DisruptTrack(playerid);
			SetPlayerColor(playerid,TeamColour(class_teams[player_class[playerid]]));  // for radar visibility
			SetPlayerPosAtSelf(playerid);
			return 1;
