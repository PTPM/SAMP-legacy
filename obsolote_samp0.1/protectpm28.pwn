new mode_name[] = "Protect the Prime Minister 2.8";
...
public OnGameModeInit()
{
	...
	if (map==0) {
		...
		AddStaticPickup(371,2,1529.0437,-1364.1820,327.8988); // parachute
...
public OnPlayerConnect(playerid)
{
	...
	SendClientMessage(playerid,COLOUR_IMPORTANT,"(PM Can no longer drive sports bikes!)");
	SendClientMessage(playerid,COLOUR_IMPORTANT,"(More spawn locations to avoid telefragging)");
	SendClientMessage(playerid,COLOUR_IMPORTANT,"(NEW ADMIN SYSTEM!)");
	...
	for (new i=0 ; i<MAX_PLAYERS ; i++) {
		if (IsPlayerConnected(i) && player_class[i]!=INVALID_CLASS) {
			if (player_vehicle_driver[i]) {
				...
				if (class_teams[player_class[i]]==TEAM_PRIMEMINISTER && (
					vehicle_modelid[vehicleid] == BIKEFAST || vehicle_modelid[vehicleid] == BIKESLOW)) {
					GameTextForPlayer(i,"~y~The PM is ~r~not qualified~n~ ~y~to use this bike!",3000,4);
					RemovePlayerFromVehicle(i);
				}
			}
