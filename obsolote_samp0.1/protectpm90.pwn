new mode_name[] = "Protect the Prime Minister 9.0";
...
public OnPlayerConnect(playerid)
{
	...
	SendClientMessage(playerid,COLOUR_IMPORTANT,"(various fixes and changes (various artists))");
	SendClientMessage(playerid,COLOUR_IMPORTANT,"(workaround for login bug)");
...
public OnPlayerCommandText(playerid, cmdtext[])
{
	...
	if (numparams==0) {}
 	...
 	if (streq(cmd,"/reclass")) {
		...
		for (new i=0 ; i<playerid_max ; i++){
        	if (IsPlayerConnected(i) && IsPlayerOp(i)) {
					    SendClientMessage(i, COLOUR_PERSONAL, text);
			}
		}
		return 1;
	...
	if (IsPlayerOp(playerid)) {
		if (streq(cmd,"/kick")) {
			...
			format(text, sizeof text, "%s was kicked from the server", suspectname);
			if(reason[0] != 0){
