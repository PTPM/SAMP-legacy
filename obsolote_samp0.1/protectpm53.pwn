new mode_name[] = "Protect the Prime Minister 5.3";
...
{
	SendClientMessage(playerid,COLOUR_PERSONAL,"Usage: /mapvote ls|sf|desert|a51|lv|air");
}
...
public OnPlayerConnect(playerid)
{
	...
	SendClientMessage(playerid,COLOUR_IMPORTANT,"(medics: can now heal patients in the same car)");
	SendClientMessage(playerid,COLOUR_IMPORTANT,"(medics: use /h instead of /heal <person> to heal nearest person)");
	SendClientMessage(playerid,COLOUR_IMPORTANT,"(/pinfo now gives minigun status");
...
PlayerHealPlayer(medic, patient)
{
	...
	if (DistanceFromPlayerToPlayer(medic,patient)>2) {
