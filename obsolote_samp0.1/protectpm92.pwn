new mode_name[] = "Protect the Prime Minister 9.2";
...
// do while requires so we can do if (...) log_printf(...); else ...
...
AddBannedCuboid(Float:max_x, Float:min_x, Float:max_y, Float:min_y, Float:max_z, Float:min_z, message[], Float:tele_x, Float:tele_y, Float:tele_z, Float:tele_a)
{
...
{
	SendClientMessage(playerid,COLOUR_PERSONAL,"Usage: /mapvote ls_hydra|sf|desert|a51|lv_obj|air|chiliad|country|ls|lv|fac");
}
...
public PlayerMapVoteStatus(playerid)
{
	...
	new text[256];
	format(text, sizeof text,"Current scores: ls_hydra:%d, sf:%d, desert:%d, a51:%d, lv_obj:%d, air:%d chiliad:%d country:%d ls:%d lv:%d factory:%d", score[0], score[1], score[2], score[3], score[4],score[5],score[6],score[7],score[8],score[9],score[10]);
...
public OnGameModeInit()
{
	...
	if (map==0) {
		...
		AddWeaponPickup(PICKUP_SHOTGSPA,WEAPON_SHOTGSPA,100,2021.327,1013.349,10.3203,30,WP_SYNCED); // 85 Los Santos
...
//		AddStaticPickup(PICKUP_PARACHUTE,2,1529.0437,-1364.1820,327.8988); // parachute
		...
		AddBannedCuboid(-2328,-2330,1530,1528,1,-1,"",-2301.4436,1545.0177,18.7734,90);
		...
		AddBannedCuboid(230,225,1874,1869,14,0,"~w~Not allowed there.",213.3158,1868.9786,13.1406,0);

		AddTask(614.88,1257.94,33.78,15.0,TASK_MINI,90);
		AddTask(609.08,875.71,-46,100.0,TASK_KEYS,60);
		AddTask(-420.45,1370.42,13.00,8.0,TASK_HP_BONUS,90);
		AddTask(211.66,1810.98,21.87,6.0,TASK_SAFEHOUSE,60);
...
//		AddBannedCuboid(230,225,1874,1869,14,0,"",248.8800,1849.9883,8.7734,0);
		...
		AddBannedCuboid(97,82,1942,1798,500,17.6,"",97.5643,1920.4034,18.1709,267.3321);
		AddBannedCuboid(293,82,1962,1942,500,17.6,"",232.8058,1927.2909,17.6406,181.1084);
		AddBannedCuboid(293,286,1942,1798,500,17.6,"",282.9717,1820.8510,17.6406,89.6375);
		AddBannedCuboid(286,82,1798,1780,500,17.6,"",210.5844,1801.2509,17.6406,355.0099);
		AddBannedCuboid(293,97,1961,1789,300,75,"",213.6958,1874.7875,17.6406,174.5749);
...
//		intel_feature = "Factory";
		...
		AddBannedCuboid(2704,2703,2677,2676,13,12,"",2703.1545,2678.8079,22.9472,90); // lump up
		AddBannedCuboid(2703,2701,2678,2676,25,23,"",2703.1213,2674.4536,10.8203,177); // lump down
		AddBannedCuboid(2588,2587,2639,2638,11,10,"",2589.4565,2638.3140,109.1749,267); //tower 1 up
		AddBannedCuboid(2597,2596,2639,2638,110,109,"",2585.5278,2638.7070,10.8203,86); // tower 1 down
		AddBannedCuboid(2501,2500,2691,2690,11,10,"",2503.0115,2690.4963,74.8281,267); // tower 2 up
		AddBannedCuboid(2502,2501,2691,2690,77,76,"",2503.3506,2690.4451,10.8130,183); // tower 2 down
		AddBannedCuboid(2713,2712,2774,2773,11,10,"",2715.1074,2773.7234,74.8281,266); // tower 3 up
		AddBannedCuboid(2714,2713,2774,2773,77,76,"",2715.3704,2773.9868,10.8203,183); // tower 3 down
		AddBannedCuboid(2633,2632,2836,2835,25,24,"",2632.4780,2835.6714,122.9219,185); // tower 4 up
		AddBannedCuboid(2633,2632,2830,2829,123,122,"",2632.7471,2837.5491,21.3222,359); // tower 4 down
//        AddBannedCuboid(2563,2562,2725,2724,13,12,"",2562.4490,2721.8955,22.9507,180); // jump up
        AddBannedCuboid(2563,2562,2724,2723,25,24,"",2562.5586,2726.2371,10.9844,357); // jump down
		AddBannedCuboid(2658,2657,2644,2643,37,36,"",2657.2871,2640.6255,10.8203,182); // vat 1 down
		AddBannedCuboid(2658,2657,2644,2643,11,10,"",2657.7510,2646.1182,35.2998,358); // vat 1 up
		AddBannedCuboid(2572,2571,2644,2643,11,10,"",2571.1497,2646.0725,35.2054,354); // vat 2 up
		AddBannedCuboid(2572,2571,2644,2643,37,36,"",2571.2136,2639.6768,10.8203,182); // vat 2 down
		AddBannedCuboid(2636,2635,2833,2832,25,24,"",2635.3770,2832.2854,40.3281,87); // build up
		AddBannedCuboid(2608,2607,2803,2802,24,23,"",2607.3660,2803.6421,27.8203,0); // quad up
		AddBannedCuboid(2595,2594,2705,2704,26,25,"",2582.8018,2703.7966,22.9507,89); // jump
		AddBannedCuboid(2570,2498,2617,2605,50,0,"~w~Not allowed there.",2534.9709,2654.6030,10.8203,310);
		AddBannedCuboid(2498,2487,2690,2617,50,0,"~w~Not allowed there.",2534.9709,2654.6030,10.8203,310);
		AddBannedCuboid(2751,2702,2665,2617,50,0,"~w~Not allowed there.",2700.0085,2667.0061,10.8203,40);
...
// AddSecurityCamera();
		...
		distance_display_interval = 10;
  		...
  		AddTeamSpawn(TEAM_PSYCHO, 2563.3438,2848.0994,19.9922,90);
        AddTeamSpawn(TEAM_PSYCHO, 2544.5574,2805.5715,19.9922,90);
        AddTeamSpawn(TEAM_PSYCHO, 2608.1934,2825.5627,19.9922,90);
...
PrintMapContent(map)
{
	...
	// safe zone
	if (safehouse_exists) {
	    format(text,sizeof text,"dot %f %f %f rgba(255,0,0,50)",safehouse_x,safehouse_y,safehouse_exclusion*2);
	    fwrite(file,text);
	}
	...
	for (new i=0 ; i<task_counter ; i++) {
	    format(text,sizeof text,"dot %f %f %f rgba(128,128,0,50)\n",task_x[i],task_y[i],task_size[i]);
	    fwrite(file,text);
	}

	// vehicles
	for (new i=1 ; i<vehicle_counter ; i++) {
	    format(text,sizeof text,"dot %f %f 3 black\n",vehicle_spawn_x[i],vehicle_spawn_y[i]);
	    fwrite(file,text);
	}

	// pickups
	for (new i=0 ; i<pickup_counter ; i++) {
	    format(text,sizeof text,"dot %f %f 3 red\n",pickup_x[i],pickup_y[i]);
	    fwrite(file,text);
	}

	// boundaries
    format(text,sizeof text,"line %f %f %f %f 2 rgba(0,0,0,128)\n",game_boundary_min_x,game_boundary_min_y,game_boundary_max_x,game_boundary_min_y);
    fwrite(file,text);
    format(text,sizeof text,"line %f %f %f %f 2 rgba(0,0,0,128)\n",game_boundary_min_x,game_boundary_min_y,game_boundary_min_x,game_boundary_max_y);
    fwrite(file,text);
    format(text,sizeof text,"line %f %f %f %f 2 rgba(0,0,0,128)\n",game_boundary_max_x,game_boundary_max_y,game_boundary_max_x,game_boundary_min_y);
    fwrite(file,text);
    format(text,sizeof text,"line %f %f %f %f 2 rgba(0,0,0,128)\n",game_boundary_max_x,game_boundary_max_y,game_boundary_min_x,game_boundary_max_y);
    fwrite(file,text);
...
public OnPlayerConnect(playerid)
{
	...
	SendClientMessage(playerid,COLOUR_IMPORTANT,"(NEW: FACTORY MAP (rambo,r3mp)!!!)");
	SendClientMessage(playerid,COLOUR_IMPORTANT,"(NEW: bigger a51 boundaries (puppyluv))");
	SendClientMessage(playerid,COLOUR_IMPORTANT,"(fixed /camoff)");
	SendClientMessage(playerid,COLOUR_IMPORTANT,"(removed sf minigun)");
	SendClientMessage(playerid,COLOUR_IMPORTANT,"(can't reclass if frozen)");
...
SetPlayerClass(playerid, class)
{
	...
	if (player_class[playerid]!=INVALID_CLASS) {
		...
		if (NumPlayersConnected()<=8) SendClientMessageToAll(ClassColour(player_class[playerid]),text);
	}
	...
	if (NumPlayersConnected()<=8) SendClientMessageToAll(ClassColour(class),text);
...
public RoundTick()
{
	...
	if (pm_health_bonus && pm!=INVALID_PLAYER_ID && round_timer_counter % 5 == 0) { // 1% health every 5 seconds
		...
		if (health < 100) {
			SetPlayerHealth(pm,health+pm_health_bonus);
		}
	}
...
public OnPlayerCommandText(playerid, cmdtext[])
{
	...
	if (numparams==0) {}
 	...
 	if (streq(cmd,"/reclass")) {
		...
		SendClientMessage(playerid, COLOUR_PERSONAL, "/logout <user> <pass> /camoff");
	...
	if (IsPlayerOp(playerid)) {
		if (streq(cmd,"/kick")) {
			...
			GetPlayerName(id, kicker, sizeof kicker);
			...
			new id=strvalfixed(rest);
			DisruptTrack(playerid);
			SetPlayerPos(playerid,task_x[id],task_y[id],task_z[id]);
			SendClientMessage(playerid, COLOUR_PERSONAL, "Done.");
			return 1;
			...
			new id=strvalfixed(rest);
			DisruptTrack(playerid);
			SetPlayerPos(playerid,cam_pickup_x[id],cam_pickup_y[id],cam_pickup_z[id]);
			SendClientMessage(playerid, COLOUR_PERSONAL, "Done.");
			return 1;
...
ResetPlayer(playerid)
{
	...
	DisruptTrack(playerid);
	SetPlayerPos(playerid,2236.5259,-1073.7905,1049.0234);
			...
			if (player_vehicle_passenger[i]) {
	 		...
	 		for (new j=0 ; j<banned_cuboid_counter ; j++) {
		    	if (x>=banned_cuboid_min_x[j] &&
					...
					DisruptTrack(i);
					SetPlayerPos(i,banned_cuboid_tele_x[j],banned_cuboid_tele_y[j],banned_cuboid_tele_z[j]);
			    ...
			    new Float:d = floatsqroot(floatpower(x-cam_pickup_x[j],2) + floatpower(y-cam_pickup_y[j],2) + floatpower(z-cam_pickup_z[j],2));
			    	if (d<1.5) {
						if (strlen(cam_view_message[j])>0)
   						...
   						DisruptTrack(i);
   						SetPlayerPos(i,cam_player_busy_x[j],cam_player_busy_y[j],cam_player_busy_z[j]);
			...
			if (player_reset_me[i]) {
				if (x>=253.9448 &&
					...
					y>=-43.8946) {
						...
						DisruptTrack(i);
						SetPlayerPos(i,player_reset_x[i],player_reset_y[i],player_reset_z[i]);
  						player_reset_me[i] = 0;
				}
			}
	...
	if (sd<100) {
	    ...
	    if (player_vehicle_driver[playerid]) {
	        ...
	        if (speedkph > maxspeed*1.25 && player_last_speed_warning[playerid] < round_timer_counter - 5) {
			    format (text, sizeof text, "%s (%d) is going rather fast (%.0f kph) %s max: %.0f kph (STILL IN TESTING)",text,playerid,speedkph,vehicle_name[vehicle_modelid[GetPlayerVehicleID(playerid)]-400],maxspeed);
			    for (new i=0 ; i<playerid_max ; i++) {
					if (IsPlayerConnected(i) && IsPlayerOp(i)) {
