new mode_name[] = "Protect the Prime Minister 3.5";
...
public PlayerMapVoteStatus(playerid)
{
	...
	new text[256];
	format(text, sizeof text,"Current scores: ls %d, sf %d, desert %d, a51: %d", score[0], score[1], score[2], score[3]);
...
public OnGameModeInit()
{
	...
	if (map==0) {
		...
		AddWeaponPickup(PICKUP_UZI,WEAPON_UZI,150,162.0741,1934.7688,33.8984); // Uzis
		...
		AddStaticPickup(PICKUP_ARMOR,3,244.6849,1858.7109,14.0840); // cop armour
		AddStaticPickup(PICKUP_JETPACK,3,268.4451,1884.2373,-30.0938); // jetpack
		AddStaticPickup(PICKUP_ARMOR,3,268.5153,1891.2520,-12.8603); // armour bottom (terrorist)

		AddWeaponPickup(PICKUP_FLAMETHROWER,WEAPON_FLAMETHROWER,50,263.0227,1878.9073,-30.3906,WP_RESPAWN,WP_SYNCED); // flame thrower (right at the bottom)
		AddWeaponPickup(PICKUP_UZI,WEAPON_UZI,80,276.5279,1886.5530,8.4375,WP_RESPAWN,WP_SYNCED); // uzi jetpack top

		AddWeaponPickup(PICKUP_TEC9,WEAPON_TEC9,80,271.6212,1873.7330,8.7578,WP_NO_RESPAWN,WP_UNSYNCED); // uzi terrorist

		AddWeaponPickup(PICKUP_SAWEDOFF,WEAPON_SAWEDOFF,24,330.1199,1848.1210,7.7266,WP_RESPAWN,WP_SYNCED); // sawnoff
		AddWeaponPickup(PICKUP_SAWEDOFF,WEAPON_SAWEDOFF,24,287.9067,1824.1339,7.7340,WP_RESPAWN,WP_SYNCED); // sawnoff

		AddWeaponPickup(PICKUP_UZI,WEAPON_UZI,80,275.7653,1841.6360,9.3473,WP_RESPAWN,WP_SYNCED); // uzi on duct (in corridor)
		AddWeaponPickup(PICKUP_UZI,WEAPON_UZI,80,293.8852,1820.1926,4.7109,WP_RESPAWN,WP_SYNCED); // uzi under stairs
		AddWeaponPickup(PICKUP_AK47,WEAPON_AK47,80,272.7304,1816.0725,1.0078,WP_RESPAWN,WP_SYNCED); // ak

		AddWeaponPickup(PICKUP_UZI,WEAPON_UZI,80,245.4331,1818.4679,7.5547,WP_RESPAWN,WP_SYNCED); // top room middle
		AddWeaponPickup(PICKUP_SHOTGSPA,WEAPON_SHOTGSPA,24,239.1696,1834.3403,7.0865,WP_RESPAWN,WP_SYNCED); // shotgspa
		AddWeaponPickup(PICKUP_FLAMETHROWER,WEAPON_FLAMETHROWER,50,246.3053,1830.4159,12.2121,WP_RESPAWN,WP_SYNCED); // nice uzi top of pipe
		AddWeaponPickup(PICKUP_MP5,WEAPON_MP5,80,239.4735,1836.7117,10.7606,WP_RESPAWN,WP_SYNCED); // mp5
...
public OnPlayerConnect(playerid)
{
	...
	SendClientMessage(playerid,COLOUR_IMPORTANT,"(More vehicles on the desert map)");
	SendClientMessage(playerid,COLOUR_IMPORTANT,"(Area51 map called \"a51\")");
