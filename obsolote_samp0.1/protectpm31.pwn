new mode_name[] = "Protect the Prime Minister 3.1";
...
public OnPlayerConnect(playerid)
{
	...
	SendClientMessage(playerid,COLOUR_IMPORTANT,"(Map voting) - /mapvote for more info");
	SendClientMessage(playerid,COLOUR_IMPORTANT,"(Brighter colours for medics)");
...
public OnPlayerCommandText(playerid, cmdtext[])
{
	...
	if (player_op[playerid]) {
		if (streq(cmd,"/kick")) {
			...
			SendClientMessage(playerid, COLOUR_PERSONAL, text);
			return 1;
		} else if (streq(cmd,"/setmapvote")) {
			new idtext[256], mapid[256];
			parse_command(rest,idtext,mapid);
			new id = strval(idtext);
			new map = strval(mapid);
			player_map[id] = map;
			return 1;
		} else if (streq(cmd,"/getprop")) {
			new text[256];
			format (text,sizeof text,"voted map: %d",MapVoteGetVotedMap());
			SendClientMessage(playerid, COLOUR_PERSONAL, text);
			return 1;
		} else if (streq(cmd,"/setprop")) {
		    MapVoteStore();
			SendClientMessage(playerid, COLOUR_PERSONAL, "Done.");
			return 1;
