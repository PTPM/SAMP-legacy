...
new mode_name[] = "Protect the Prime Minister 2.0";
...
public OnPlayerConnect(playerid)
{
	...
	SendClientMessage(playerid,COLOUR_IMPORTANT,"(Added team balancing)");
...
public OnPlayerRequestClass(playerid, classid)
{
	...
	if (VetoPlayerClass(classid, player_class[playerid]) == classid) {
...
Balancedness(proposedclassid, oldclassid, &dualpm)
{
...
VetoPlayerClass(classid, oldclassid)
{
	...
	new prebalancedness = Balancedness(-1, -1,dualpm);
	new postbalancedness = Balancedness(classid, oldclassid, dualpm);
...
/*
	new text[256];
	format(text, sizeof text, "players_count: %d was %d now %d (%d -> %d)",players_count,prebalancedness,postbalancedness,oldclassid,classid);
	SendClientMessageToAll(0xFFFFFFAA,text);
*/

	if (!dualpm && (players_count<=5 || postbalancedness<=10 || postbalancedness < prebalancedness)) {
	    return classid;
	}
	...
	switch (class_teams[classid]) {
		...
		case TEAM_TERRORIST: {
			new class_trampx = class_tramp1 + random(NUM_TRAMPS-1);
			return class_trampx;
		}
		case TEAM_PRIMEMINISTER: {
		    // spawning as pm always works unless theres already a pm
		    if (!dualpm) return classid;
			new class_bodyguardx = class_bodyguard1 + random(NUM_BODYGUARDS-1);
			return VetoPlayerClass(class_bodyguardx, oldclassid);
		}
		case TEAM_BODYGUARD: {
			new class_copx = class_cop1 + random(NUM_COPS-1);
...
public OnPlayerCommandText(playerid, cmdtext[])
{
	...
	if (numparams==0) {}
 	...
 	if (streq(cmd,"/reclass")) {
		...
		if (strlen(rest)==0) {
		    ...
		    myclass = class_cop1 + random(NUM_COPS-1);
		} else if (streq(rest,"terrorist") || streq(rest,"t")) {
		    myclass = class_terrorist1 + random(NUM_TERRORISTS-1);
		} else if (streq(rest,"bodyguard") || streq(rest,"b")) {
		    myclass = class_bodyguard1 + random(NUM_BODYGUARDS-1);
		} else if (streq(rest,"psycho") || streq(rest,"p")) {
		    myclass = class_tramp1 + random(NUM_TRAMPS-1);
		...
		if (VetoPlayerClass(myclass,player_class[playerid])==myclass) {
	...
	if (IsPlayerAdmin(playerid)) {
		if (streq(cmd,"/kick")) {
			...
			new id[256], class[256];
			parse_command(rest,id,class);
			player_class[strval(id)] = strval(class);
			new text[256];
			format (text,sizeof text,"player_class[%d] = %d",strval(id), player_class[strval(id)]);
			SendClientMessage(playerid, COLOUR_PERSONAL, text);
			return 1;
		} else if (streq(cmd,"/get")) {
			new text[256];
			format (text,sizeof text,"player_class[%d] = %d",strval(rest), player_class[strval(rest)]);
			SendClientMessage(playerid, COLOUR_PERSONAL, text);
			return 1;
		} else if (streq(cmd,"/balancedness")) {
		    new dualpm = 0;
		    new b = Balancedness(-1, -1, dualpm);
			new text[256];
			format (text,sizeof text,"balancedness: %d, dualpm=%d",b,dualpm);
			SendClientMessage(playerid, COLOUR_PERSONAL, text);
			return 1;
