new mode_name[] = "Protect the Prime Minister 7.0";
...
{
	printf("New Task (%f,%f,%f) %f %d %d.",x,y,z,size,type,time);
...
public OnPlayerConnect(playerid)
{
	...
	SendClientMessage(playerid,COLOUR_IMPORTANT,"(COUNTRYSIDE MAP BY R3MP!)");
	SendClientMessage(playerid,COLOUR_IMPORTANT,"(PM: Look out for tasks around the map)");
	SendClientMessage(playerid,COLOUR_IMPORTANT,"(coming soon: r3mp's chiliad improvements, new desert boundaries)");
...
*/
	...
	if (pm_radar_time>=0) pm_radar_time--;
	if  (pm_radar_time == 0) {
